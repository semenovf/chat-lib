...
std::shared_ptr<ContactList> ContactListBuilder::operator () ()
{
    ...
    if (!fs::exists(contactListPath)) {
        ...
        if (!fs::create_directory(contactListPath, ec)) {
            on_failure(fmt::format("Create directory failure: {}: {}"
                , PFS_UTF8_ENCODE_PATH(contactListPath.c_str())
                , ec.message()));
            return nullptr;
        }
    }
    ...
    type::underlying_type contactList {dbh, on_failure};
    ...
    auto success = contactList.add(forward_iterator{SAMPLE_DATA}
